// ==UserScript==
// @name         超星学习通、知到智慧树、雨课堂、职教云icve、超星考试、学堂云4.0
// @namespace    Mr.Zane
// @version      1.2.6
...
// @grant        GM_setValue
// @compatible   chrome
// @contributionURL   http://juan.77zane.xyz/
...
var setting = {
        ...
        //雨课堂功能配置区域（不使用可不配置）-->本区域登陆雨课堂都要修改-->确保sessionid正确
...
if (url == '/mycourse/studentcourse'){
    ...
    _self.layui.use('layer', function() {
        this.layer.open({
            content: '收费题库并非本人管理，不建议购买，可使用公众号查题替代<br>无法保证程序不出现意外<br>不保证及时维护<br>点击同意即获取用户识别信息',
            title: '用户协议',
            btn: ['我同意'],
            btn1:function(){
...
function beforeFind() {
    ...
    setting.div = $$(
        '<div style="box-shadow: darkgrey 10px 10px 10px 5px;border: 0.5px solid #666;width: 330px;background-color: #86CCCA; position: fixed; top: 100px; right: 0; z-index: 99999;">' +
        '<span style="font-size: medium;"></span>' +
        '<img src="https://s3.ax1x.com/2020/12/21/rdddV1.png" width="100%" />'+
        '设置token:<input id="phone" type="text name="phone" value='+GM_getValue('phone1')+'>'+
        '<button id="button" type="submit">确定</button>'+
        '<div style="color: bule;background: #B0E2FF;;font-size: 15px;margin-bottom: 1%;">目前默认搜索|'+(userSetting.option ? '收费题库':'免费题库')+'<br>脚本完全免费<br>如无答案请移步公众号查询</div>' +
        ...
        '<button  style="background-color: #4CAF50;border: none;margin: 1%;color: white;width: 70px;height: 30px;text-align: center;text-decoration: none;display: inline-block;font-size: 10px;">向我捐赠</button>' +
        '<button  style="background-color: #4CAF50;border: none;margin: 1%;color: white;width: 70px;height: 30px;text-align: center;text-decoration: none;display: inline-block;font-size: 10px;">反馈QQ群</button>' +
        '<button  style="background-color: #4CAF50;border: none;margin: 1%;color: white;width: 70px;height: 30px;text-align: center;text-decoration: none;display: inline-block;font-size: 10px;">福利线报群</button>' +
        '<div style="max-height: 300px; overflow-y: auto;">' +
        '<table border="1"  style="font-size: 12px;color:black;background-color:#86CCCA;">' +
        '<thead>' +
        '<tr>' +
        ...
        if (this.nodeName == 'TD') {
            ...
            window.open("http://juan.77zane.xyz/");
        } else if (len == 6){
            window.open('https://qm.qq.com/cgi-bin/qm/qr?k=kH5llabsDSZ5bmXJtGkjaLGzx_BWPtom&jump_from=webapi')
        } else if (len == 7){
            window.open('https://qm.qq.com/cgi-bin/qm/qr?k=ooMcWxFGKJCr8P_X-pIB_lwhZ0Tfntgf&jump_from=webapi')
        }
    ...
    GM_xmlhttpRequest({
        ...
        onload: function(xhr) {
            if (!setting.loop) {
                ...
                if (obj.code == 200) {
                    userSetting.option ? setting.div.children('div:eq(0)').text('正在搜索收费题库--当前积分剩余：'+obj.data.num):setting.div.children('div:eq(0)').text('正在搜索免费题库|防止失联建议加群'+'1143643829');
                ...
                setting.div.children('div:eq(0)').html('发现未知异常，请联系加群1143643829反馈处理');
            }
        },
        ontimeout: function() {
            setting.loop && setting.div.children('div:eq(0)').html(setting.over + '正在链接到云端，请稍后....<br>如果长时间链接不上请加群1143643829反馈');
        }
    });
}
...
function ZHSbeforeFind() {
    setting.div = $$(
        '<div id = "setting" style="box-shadow: darkgrey 10px 10px 10px 5px;border: 0.5px solid #666;width: 300px;background-color: #86CCCA; position: absolute; top: 0; right: 0; z-index: 99999;">' +
        '<span style="font-size: medium;"></span>' +
        '<img src="https://s3.ax1x.com/2020/12/21/rdddV1.png" width="100%" />'+
        '设置token:<input id="phonenum" type="text" name="phone" value=' + GM_getValue('phone1')+'>'+
        '<button id="button" type="submit">确定</button>'+
        '<div style="color: bule;background: #B0E2FF;font-size: 15px;margin-bottom: 1%;">目前默认搜索|'+(userSetting.option ? '收费题库':'免费题库')+'<br>脚本完全免费<br>如无答案请移步公众号查询</div>' +
    ...
    GM_xmlhttpRequest({
        ...
        onload: function(xhr) {
            if (!setting.loop) {
                ...
                var obj = $$.parseJSON(xhr.responseText) || {};
                if (obj.code == 200) {
                    setting.div.children('div:eq(0)').text((!userSetting.option ? '正在搜索|答案不对加群反馈'+'1143643829' : '正在搜索|当前积分剩余'+obj.data.num));
                    ...
                    setting.div.children('div:eq(0)').html('出现未知异常，请加群1143643829反馈');
                }
            } else if (xhr.status == 404) {
                setting.div.children('div:eq(0)').data('html', '最新版本已发布！请点击油猴按钮【用户脚本检测更新】完成更新。').siblings('button:eq(1)').click();
            }else {
                setting.div.children('div:eq(0)').text('发生未知异常，出现未知异常，请加群1143643829反馈');
            }
        },
        ontimeout: function() {
            setting.loop && setting.div.children('div:eq(0)').text('正在链接到云端，请稍后....<br>如果长时间链接不上出现未知异常，请加群1143643829反馈');
        }
    });
}
function ZhiHuiShuNewFillAnswer($option,obj,postData){
    //真的是醉了，找个智慧树账号进行测试都这么难，目前只找到了  单选题/多选题/判断题   的题型
    //如果有热心的小伙伴可以提供智慧树账号给我测试QQ1769482007
...
function ZHSfindAnswer() {
    ...
    GM_xmlhttpRequest({
        ...
        onload: function(xhr) {
            if (!setting.loop) {
                ...
                if (obj.code == 200) {
                    ...
                    setting.div.children('div:eq(0)').html('发生未知异常，出现未知异常，请加群1143643829反馈');
                }
            } else if (xhr.status == 404) {
                setting.div.children('div:eq(0)').data('html', '最新版本已发布！请点击油猴按钮【用户脚本检测更新】完成更新。').siblings('button:eq(1)').click();
            }else {
        ...
        ontimeout: function() {
            setting.loop && setting.div.children('div:eq(0)').text('正在链接到云端，请稍后....<br>如果长时间链接不上请请加群1143643829反馈');
        }
    });
}
        ...
        onload: function(xhr) {
            if (!setting.loop) {
                ...
                if (obj.code == 200) {
                    ...
                    setting.$$div.html('出现未知异常，请联系QQ1769482007');
                }
            } else if (xhr.status == 403) {
                ...
                setting.$$div.html('出现bug，请加群1143643829反馈');
            }
        },
        ontimeout: function() {
            setting.loop && setting.$$div.html('正在链接到云端，请稍后....<br>如果长时间链接不上请加群1143643829反馈');
        }
    });
}
...
function Exam(){
    setting.div = $$(
        '<div id = "setting" style="border: 2px dashed rgb(0, 85, 68); width: 330px; position: fixed; top: 0; right: 0; z-index: 99999; background-color: #86CCCA; overflow-x: auto;">' +
        '<span style="font-size: medium;"></span>' +
        '<img src="https://s3.ax1x.com/2020/12/21/rdddV1.png" width="100%" />'+
        '设置token:<input id="phonenum" type="text name="phone" value='+GM_getValue('phone1')+'>'+
        '<button id="button" type="submit">确定</button>'+
...
function ZhiJiaoYunBeforeFind() {
    setting.div = $$(
        '<div style="box-shadow: darkgrey 10px 10px 10px 5px;border: 0.5px solid #666;width: 330px;background-color: #86CCCA; position: fixed; top: 100px; right: 300px; z-index: 99999;">' +
        '<span style="font-size: medium;"></span>' +
        '<img src="https://s3.ax1x.com/2020/12/21/rdddV1.png" width="100%" />'+
        '设置token:<input id="phone" type="text name="phone" value='+ GM_getValue('phone1') + '>'+
        '<button id="button" type="submit">确定</button>'+
        ...
        onload: function(xhr) {
            if (!setting.loop) {
                ...
                if (obj.code == 200) {
                    ...
                    setting.div.children('div:eq(0)').html('发现未知异常，请联系请加群1143643829反馈').siblings('button:eq(1)').click();;
                }
                //setting.div.children('span').html(obj.msg || '');
            } else if (xhr.status == 404) {
                setting.div.children('div:eq(0)').data('html', '最新版已发布！请点击油猴按钮【用户脚本检查更新】完成更新！').siblings('button:eq(1)').click();
            }else {
                setting.div.children('div:eq(0)').html('发现未知异常，请加群1143643829反馈');
            }
        },
        ontimeout: function() {
            setting.loop && setting.div.children('div:eq(0)').html(setting.over + '正在链接到云端，请稍后....<br>如果长时间链接不上请加群1143643829反馈');
        ...
        onload: function(xhr) {
            if (!setting.loop) {
                ...
                if (obj.code == 200) {
                    ...
                    setting.div.children('div:eq(0)').html('发现未知异常，请加群1143643829反馈').siblings('button:eq(1)').click();;
                }
                //setting.div.children('span').html(obj.msg || '');
            } else if (xhr.status == 403) {
                setting.div.children('div:eq(0)').data('html', '由于你的请求过于频繁，您的ip已被封<br>10分钟后自动解封<br>或者使用收费答题不限制请求次数').siblings('button:eq(1)').click();
    ...
    GM_xmlhttpRequest({
        ...
        onload: function(xhr) {
            if (!setting.loop) {
                ...
                var obj = $$.parseJSON(xhr.responseText) || {};
                if (obj.code == 200) {
                    userSetting.option ? setting.div.children('div:eq(0)').text('正在搜索token题库--当前积分剩余：'+obj.data.num):setting.div.children('div:eq(0)').text('正在搜索免费题库|防止失联建议加群'+'1143643829');
                    ...
                    setting.div.children('div:eq(0)').html('发现未知异常，请联系QQ1769482007报告反馈bug').siblings('button:eq(1)').click();;
                }
                //setting.div.children('span').html(obj.msg || '');
            } else if (xhr.status == 403) {
                ...
                setting.div.children('div:eq(0)').html('发现未知异常，请联系QQ1769482007处理');
            }
        },
        ontimeout: function() {
            setting.loop && setting.div.children('div:eq(0)').html(setting.over + '正在链接到云端，请稍后....<br>如果长时间链接不上请联系QQ1769482007');
        }
    });
}
function YuKeTangBeforeFind(){
    setting.div = $(
        ...
        '<span style="font-size: medium;"></span>' +
        '<img src="https://s3.ax1x.com/2020/12/21/rdddV1.png" width="100%" />'+
        '设置token(手机号):<input id="phone" type="text name="token" value=' + GM_getValue('phone1') + '>'+
        '<button id="button" type="submit">确定</button>'+
        '<div style="color: bule;background: #B0E2FF;;font-size: 15px;margin-bottom: 1%;">目前默认搜索|'+(userSetting.option ? 'token题库':'免费题库')+'<br>脚本完全免费<br>如无答案请移步公众号查询</div>' +
        '<button  style="background-color: #4CAF50;border: none;margin: 1%;color: white;width: 60px;height: 30px;text-align: center;text-decoration: none;display: inline-block;font-size: 10px;">暂停答题</button> ' +
        '<button  style="background-color: #4CAF50;border: none;margin: 1%;color: white;width: 110px;height: 30px;text-align: center;text-decoration: none;display: inline-block;font-size: 10px;">' + (setting.auto ? '取消本次自动提交' : '开启本次自动提交') + '</button>' +
        ...
        onload: function(xhr){
            ...
            if (obj.code == 200){
                ...
                if(obj.code == 101){
                    ...
                    setting.div.children('div:eq(0)').html("我们帮你查询到了答案，但是你的积分不足，请充值之后再进行查询，点击下方按钮【向我捐赠】")
                    addAnswer(1,obj.msg,'');
                }else{
                    setting.div.children('div:eq(0)').html("发生未知异常，请刷新页面重新获取，还是不行请联系作者qq：1769482007进行报告反馈bug，感谢你的支持")
                }
            }
        }
    })
...
function YKTfindAnswer(){
    setting.div = $(
        '<div style="box-shadow: darkgrey 10px 10px 10px 5px;border: 0.5px solid #666;width: 350px;background-color: #86CCCA; position: fixed; top: 100px; right: 0; z-index: 99999;">' +
        '<span style="font-size: medium;"></span>' +
        '<img src="https://s3.ax1x.com/2020/12/21/rdddV1.png" width="100%" />'+
        '设置token(手机号):<input id="phone" type="text name="token" value='+GM_getValue('phone1')+'>'+
        '<button id="button" type="submit">确定</button>'+
        ...
        '<button  style="background-color: #4CAF50;/* Green */border: none;margin: 1%;color: white;width: 70px;height: 30px;text-align: center;text-decoration: none;display: inline-block;font-size: 10px;">向我捐赠</button>' +
        '<button  style="background-color: #4CAF50;border: none;margin: 1%;color: white;width: 70px;height: 30px;text-align: center;text-decoration: none;display: inline-block;font-size: 10px;">反馈QQ群</button>' +
        '<button  style="background-color: #4CAF50;border: none;margin: 1%;color: white;width: 70px;height: 30px;text-align: center;text-decoration: none;display: inline-block;font-size: 10px;">福利线报群</button>' +
        '<div style="max-height: 300px; overflow-y: auto;">' +
        '<table border="1"  style="font-size: 12px;color:black;background-color:#86CCCA;">' +
